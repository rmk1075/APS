...
public class Main {
    static int srcX, srcY, ans = 0, picked[] = new int[5], visited[] = new int[5], dx[] = {-1, 0, 1, 0}, dy[] = {0, -1, 0 ,1};
    ...
    public static void dfs(int som, int count, int start) {
        ...
        for(int i = srcX; i < 5; i++) {
            for(int j = 0; j < 5; j++) {
                if(i*5 + j <= start || (picked[i] & (1 << j)) != 0) continue;
                picked[i] |= 1 << j;
    ...
    public static boolean check() {
        ...
        while(!queue.isEmpty()) {
            ...
            for(int d = 0; d < 4; d++) {
                ...
                if(x < 0 || y < 0 || x == 5 || y == 5 || (picked[x] & (1 << y)) == 0 || (visited[x] & (1 << y)) != 0) continue;
                visited[x] |= 1 << y;
                cnt++;
